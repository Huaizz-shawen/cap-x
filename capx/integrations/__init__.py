...
def _register_franka_apis() -> None:
    ...
    register_api(
        ...
        lambda env: FrankaControlApiReducedSam3(env, bimanual=True),
    )
    register_api(
        "FrankaControlApiReducedExamplelessBimanual",
        lambda env: FrankaControlApiReducedSam3Exampleless(env, bimanual=True),
    )
    register_api(
        "FrankaControlApiReducedBimanualHandover",
        lambda env: FrankaControlApiReducedSam3(env, bimanual=True, is_handover=True),
    )
    register_api(
        "FrankaControlApiReducedExamplelessBimanualHandover",
        lambda env: FrankaControlApiReducedSam3Exampleless(env, bimanual=True, is_handover=True),
    )
    register_api(
...
def _register_libero_apis() -> None:
    try:
...
def _register_robocasa_gr1_apis() -> None:
    try:
        from .robocasa import GR1RobocasaControlApi
        register_api("GR1RobocasaControlApi", GR1RobocasaControlApi)
    except ImportError:
        print("GR1RobocasaControlApi not found")


try:
    _register_franka_apis()
except ImportError as exc:
    print(f"Franka APIs not fully installed, skipping Franka API registration: {exc}")

_register_libero_apis()
_register_r1pro_apis()
_register_robocasa_gr1_apis()
