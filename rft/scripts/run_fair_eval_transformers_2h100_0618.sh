...
RFT_MODEL="${GLOBAL_ROOT}/models/grpo_lift_smoke_4h100_global_step2_hf"
OUT_ROOT="${GLOBAL_ROOT}/evals/fair_lift_transformers_pilot_2h100_pyroki_0618"
...
if ! port_ready; then
  ...
  CUDA_VISIBLE_DEVICES="" python -m capx.serving.launch_pyroki_server \
    ...
    > "${GLOBAL_ROOT}/logs/pyroki_eval_0618.log" 2>&1 &
  PYROKI_PID=$!
...
if ! port_ready; then
  ...
  tail -n 120 "${GLOBAL_ROOT}/logs/pyroki_eval_0618.log" >&2 || true
  exit 1
fi
...
root = Path("/inspire/hdd/global_user/huaizezheng-p-huaizezheng/rl-homework/evals/fair_lift_transformers_pilot_2h100_pyroki_0618")
