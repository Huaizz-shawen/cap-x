...
GLOBAL_ROOT=${GLOBAL_ROOT:-/inspire/hdd/global_user/huaizezheng-p-huaizezheng/rl-homework}
CAPX_ROOT=${CAPX_ROOT:-/inspire/hdd/global_user/huaizezheng-p-huaizezheng/cap-x}
RFT_ROOT=${RFT_ROOT:-${CAPX_ROOT}/rft}
...
DATE=${DATE:-0624}
DATA_SOURCES=${DATA_SOURCES:-franka_lift_code_env,franka_restack_code_env}
DATASET_TAG=${DATASET_TAG:-$(echo "${DATA_SOURCES}" | tr ',' '_' | tr -c '[:alnum:]_' '_')}
ALGO=${ALGO:-grpo}
GROUP_SIZE=${GROUP_SIZE:-4}
TRAIN_SIZE_PER_SOURCE=${TRAIN_SIZE_PER_SOURCE:-512}
VAL_SIZE_PER_SOURCE=${VAL_SIZE_PER_SOURCE:-64}
MODEL_PATH=${MODEL_PATH:-${GLOBAL_ROOT}/models/Qwen2.5-Coder-7B-Instruct}
DATA_ROOT=${DATA_ROOT:-${GLOBAL_ROOT}/runs/capx_grpo_multisource_${DATASET_TAG}_${DATE}}
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-${ALGO}_qwen25coder7b_multisource_${DATASET_TAG}_${DATE}_temperature_${TRAIN_TEMPERATURE}_group_size_${GROUP_SIZE}_train_per_source${TRAIN_SIZE_PER_SOURCE}_epochs${TOTAL_EPOCHS}}
...
if ! python -c "import socket; s=socket.create_connection(('127.0.0.1', ${PYROKI_PORT}), timeout=1); s.close()" 2>/dev/null; then
  ...
  CUDA_VISIBLE_DEVICES="" python -m capx.serving.launch_pyroki_server \
    ...
    > "${GLOBAL_ROOT}/logs/pyroki_train_multisource_${DATE}.log" 2>&1 &
  PYROKI_PID=$!
...
if ! python -c "import socket; s=socket.create_connection(('127.0.0.1', ${PYROKI_PORT}), timeout=1); s.close()" 2>/dev/null; then
  ...
  tail -n 120 "${GLOBAL_ROOT}/logs/pyroki_train_multisource_${DATE}.log" >&2 || true
  exit 1
fi
...
if [[ ! -f "${DATA_ROOT}/train.parquet" ]]; then
  echo "Preparing multi-source dataset..."
  python "${RFT_ROOT}/scripts/prepare_verl_multisource_dataset.py" \
    --output-dir "${DATA_ROOT}" \
    --data-sources "${DATA_SOURCES}" \
    --train-size-per-source "${TRAIN_SIZE_PER_SOURCE}" \
    --val-size-per-source "${VAL_SIZE_PER_SOURCE}" \
    --seed "${DATASET_SEED:-0}"
fi
...
python -m verl.trainer.main_ppo \
  ...
  custom_reward_function.path=${RFT_ROOT}/verl_agent_reward/hyrl_franka_strict_reward.py \
