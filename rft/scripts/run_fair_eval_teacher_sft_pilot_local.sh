...
MODEL_PATH="${MODEL_PATH:-${SCRIPT_ROOT}/models/lora_sft_lift_teacher_codex_gemini_vdm_pilot_0621_merged}"
MODEL_LABEL="${MODEL_LABEL:-teacher_sft_pilot_local}"
OUT_ROOT="${OUT_ROOT:-${SCRIPT_ROOT}/evals/local_fair_lift_teacher_sft_pilot_10seeds_0623}"
...
mkdir -p "${OUT_ROOT}" "${SCRIPT_ROOT}/.cache/robot_descriptions"

export HF_HOME="${SCRIPT_ROOT}/cache/huggingface"
export TRANSFORMERS_CACHE="${HF_HOME}"
export XDG_CACHE_HOME="${SCRIPT_ROOT}/.cache"
export MPLCONFIGDIR="${SCRIPT_ROOT}/.cache/matplotlib"
export ROBOT_DESCRIPTIONS_CACHE="${SCRIPT_ROOT}/.cache/robot_descriptions"
...
"${CAPX_VENV}/bin/python" "${SCRIPT_ROOT}/scripts/eval_capx_fair.py" \
  ...
  --gen-batch-size "${GEN_BATCH_SIZE:-2}" \
