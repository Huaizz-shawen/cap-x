...
SEARCH_SCRIPT="${GLOBAL_ROOT}/scripts/search_strict_lift_success.py"
OUT_ROOT="${OUT_ROOT:-${GLOBAL_ROOT}/evals/strict_lift_success_search_67templates_3seeds_0622}"
PYROKI_PORT="${PYROKI_PORT:-8116}"
PYROKI_LOG="${GLOBAL_ROOT}/logs/pyroki_strict_lift_success_search_0622.log"

NUM_SEEDS="${NUM_SEEDS:-3}"
MAX_TEMPLATES="${MAX_TEMPLATES:-0}"
STOP_AFTER_SUCCESSES="${STOP_AFTER_SUCCESSES:-3}"
...
if [[ ! -f "${SEARCH_SCRIPT}" ]]; then
  echo "ERROR: search script missing: ${SEARCH_SCRIPT}" >&2
  exit 1
fi
...
echo "SEARCH_SCRIPT=${SEARCH_SCRIPT}"
echo "OUT_ROOT=${OUT_ROOT}"
echo "NUM_SEEDS=${NUM_SEEDS}"
echo "MAX_TEMPLATES=${MAX_TEMPLATES}"
echo "STOP_AFTER_SUCCESSES=${STOP_AFTER_SUCCESSES}"
echo "==== GPU visibility ===="
nvidia-smi || true
