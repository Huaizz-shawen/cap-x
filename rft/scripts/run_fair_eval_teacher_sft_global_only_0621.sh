...
TEACHER_SFT_MODEL="${TEACHER_SFT_MODEL:-${GLOBAL_ROOT}/models/lora_sft_lift_teacher_codex_gemini_vdm_pilot_0621_merged}"
TEACHER_SFT_LABEL="${TEACHER_SFT_LABEL:-teacher_sft_pilot}"
OUT_ROOT="${OUT_ROOT:-${GLOBAL_ROOT}/evals/fair_lift_teacher_sft_pilot_100seeds_1h100_globalonly_0621}"
PYROKI_PORT="${PYROKI_PORT:-8116}"
PYROKI_LOG="${GLOBAL_ROOT}/logs/pyroki_eval_teacher_sft_globalonly_0621.log"
...
if [[ ! -f "${TEACHER_SFT_MODEL}/config.json" ]]; then
  echo "ERROR: expected merged teacher SFT model is missing: ${TEACHER_SFT_MODEL}" >&2
  exit 1
fi
...
echo "TEACHER_SFT_MODEL=${TEACHER_SFT_MODEL}"
echo "TEACHER_SFT_LABEL=${TEACHER_SFT_LABEL}"
echo "==== GPU visibility ===="
nvidia-smi || true
...
python "${EVAL_SCRIPT}" \
  --model-path "${TEACHER_SFT_MODEL}" \
  --output-dir "${OUT_ROOT}/teacher_sft" \
  --label "${TEACHER_SFT_LABEL}" \
  "${COMMON_ARGS[@]}"
...
teacher = json.loads((root / "teacher_sft" / "summary.json").read_text())
comparison = {
    "base_success_rate": base["success_rate"],
    "teacher_sft_success_rate": teacher["success_rate"],
    "delta_success_rate": teacher["success_rate"] - base["success_rate"],
    "base_success_count": base["success_count"],
    "teacher_sft_success_count": teacher["success_count"],
    "num_records": base["num_records"],
    "seeds": base["seeds"],
    "base_valid_execution_rate": base["valid_execution_rate"],
    "teacher_sft_valid_execution_rate": teacher["valid_execution_rate"],
    "base_mean_score": base["mean_score"],
    "teacher_sft_mean_score": teacher["mean_score"],
}
